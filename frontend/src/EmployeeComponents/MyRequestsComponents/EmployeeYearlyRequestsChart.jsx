...
const EmployeeYearlyRequestsChart = () => {
  ...
  const fetchYearlyData = async () => {
    try {
      setLoading(true);
      const response = await instance.get(`/requests/yearly-chart/me`, { params: { year: selectedYear } });
  ...
  return (
    <BaseCard padding="p-6" className="w-full mt-6 mb-6 text-left">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="font-bold text-base tracking-tight" style={{ color: 'var(--text-main)' }}>
              My Requests Analytics
            </h4>
            <span className="text-[10px] font-medium" style={{ color: 'var(--text-muted)' }}>
              Overview of your submitted requests volume per month
            </span>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <ResponsiveContainer width="100%" height={260}>
          <AreaChart data={yearlyData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
            <defs>
              <linearGradient id="neonBlueEmp" x1="0" y1="0" x2="0" y2="1">
            ...
            <Area type="monotone" dataKey="count" stroke="#0095ff" strokeWidth={2.5}
              fillOpacity={1} fill="url(#neonBlueEmp)"
