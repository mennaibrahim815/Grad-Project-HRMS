...
import { useState, useEffect } from "react";
import { useDraggable } from "@dnd-kit/core";
import { Plus, Check, X, Trash2, Edit2, Loader2, FileText } from "lucide-react"; 
import API from "@/services/axios"; 

export default function TaskCard({ id, title, description, avatar, tag, priority, onDelete, onEdit, onAddTask, assignedTo = [], documents = [] }) {
  const { attributes, listeners, setNodeRef, transform } = useDraggable({ id });
  ...
  return (
    ...
    >
      ...
      <div className="space-y-2 pt-3 border-t border-slate-700/30">
        ...
        {/* زرار إضافة مهمة فرعية ليفتح المودال الجديد مباشرة */}
        <button 
          onClick={(e) => {
            e.stopPropagation();
            onAddTask(); 
