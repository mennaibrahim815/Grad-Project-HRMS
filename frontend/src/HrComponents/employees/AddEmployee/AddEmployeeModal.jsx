...
import { validateField, isStepComplete } from "././validateEmployeeForm"
...
const AddEmployeeModal = ({ open, onClose, onSuccess }) => {
  ...
  const updateExperience = (field, value) => {
    setFormData((prev) => ({ ...prev, experience: { ...prev.experience, [field]: value } }));
  };
  ...
  const handleBlur = (section, field) => {
    ...
    setErrors((prev) => ({
      ...prev,
      [section]: { ...prev[section], [field]: errorMsg },
    }));
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      fd.append("experience[baseSalary]", Number(formData.experience.salary) || 0);
      ...
      const message = error.response?.data?.message || "Check required fields";
      setApiError({ open: true, message });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <Modal open={open} onClose={onClose}>
      <ErrorModal
        isOpen={apiError.open}
        onClose={() => setApiError({ open: false, message: "" })}
      ...
      <AddEmployeeSteps
        ...
        onStepChange={(nextStep) => {
          if (nextStep > step && !stepValid) return;
      ...
      <div className="flex flex-col h-full">
            ...
            >
              {step === 1 && (
                <GeneralStep
                  ...
                  onBlur={(field) => handleBlur("general", field)}
                />
              )}
              ...
              {step === 2 && (
                <ExperianceStep
                  ...
                  onBlur={(field) => handleBlur("experience", field)}
                />
              )}
              ...
              {step === 3 && (
                <EmployeeStep
                  ...
                  onBlur={(field) => handleBlur("employee", field)}
                />
              )}
            </motion.div>
          </AnimatePresence>
        ...
        <div className="flex items-center gap-4 px-4 pb-4 pt-3">
          ...
          <Button
            ...
            className={`flex-1 rounded-xl py-2.5 text-sm font-semibold transition flex justify-center items-center
    ${(!stepValid || isLoading) ? 'bg-gray-600 cursor-not-allowed' : 'bg-blue-500 hover:bg-blue-600'}`}
          >
            {step === 3 ? (
              isLoading ? (
