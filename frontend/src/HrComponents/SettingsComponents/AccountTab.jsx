...
const AccountTab = () => {
  ...
  if (!hr) return <div className="p-10 text-center animate-pulse text-gray-600 uppercase font-black tracking-widest">Loading HR Data...</div>;
  ...
  return (
    <div className="space-y-10">
      <section>
        ...
        <div className="flex flex-col md:flex-row items-center gap-8 mb-10">
          <div className="relative">
            <img src={formData.image || defaultAvatar} className="w-24 h-24 rounded-full object-cover border-4 border-[#0b141a] shadow-2xl" alt="avatar" />
            {!isEditing && <div className="absolute bottom-1 right-1 w-5 h-5 bg-green-500 border-4 border-[#142129] rounded-full shadow-lg"></div>}
          </div>
          ...
          <div className="flex-1 text-center md:text-left space-y-2">
            {!isEditing ? (
              <div>
                <h2 className="text-2xl font-black text-white">{hr.general?.firstName} {hr.general?.lastName}</h2>
                <p className="text-blue-500 text-xs font-bold uppercase tracking-widest">System {hr.general?.role} Manager</p>
              </div>
            ) : (
              <div className="flex gap-4 text-[10px] font-black uppercase text-blue-500">
                <button className="hover:text-white transition-colors">Change photo</button>
                <span className="text-gray-800">|</span>
                <button className="text-pink-600 hover:text-white transition-colors">Delete</button>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <InputItem label="First Name" value={formData.firstName} disabled={!isEditing} />
          <InputItem label="Last Name" value={formData.lastName} disabled={!isEditing} />
          <InputItem label="Email Address" value={formData.email} disabled={!isEditing} />
          <InputItem label="Phone Number" value={formData.phone} disabled={!isEditing} />
        </div>
      </section>
      ...
      <section className="pt-8 border-t border-gray-800/50">
        <h3 className="text-lg font-bold text-gray-200 mb-8">Security</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <InputItem label="Current Password" type="password" disabled={!isEditing} value="............" />
          <InputItem label="New Password" type="password" disabled={!isEditing} value="............" />
        </div>
      </section>
      ...
      <div className="flex justify-end gap-4 pt-10 border-t border-gray-800/20">
        {isEditing && (
          <button onClick={() => setIsEditing(false)} className="px-10 py-3 rounded-2xl border border-gray-800 text-gray-500 font-bold hover:bg-white/5 transition-all">Cancel</button>
        )}
        <button 
           onClick={() => setIsEditing(!isEditing)}
           className="px-12 py-3 rounded-2xl bg-blue-600 text-white font-bold hover:bg-blue-700 shadow-xl transition-all active:scale-95"
        >
...
const InputItem = ({ label, value, disabled, type="text" }) => (
  <div className="space-y-2">
    <label className="text-[10px] uppercase font-black text-gray-500 tracking-[0.2em] ml-1">{label}</label>
    <input 
      type={type} 
      ...
      readOnly={disabled}
      className={`w-full p-4 rounded-2xl border transition-all text-sm font-semibold outline-none ${
        disabled ? 'bg-transparent border-transparent text-gray-400' : 'bg-[#0b141a] border-gray-800 text-white focus:border-blue-500/50'
      }`} 
    />
  </div>
);
