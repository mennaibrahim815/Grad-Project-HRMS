...
import { FileText, CheckCircle2, Clock, XCircle, CalendarDays } from "lucide-react";
...
const LeaveStatsHeader = ({ onStatsUpdated }) => {
  ...
  useEffect(() => { fetchMonthlyStats(); }, [statsMonth, statsYear]);
  ...
  const statCards = [
    ...
    { title: "Approved Requests", value: stats?.approvedCount      || 0, icon: <CheckCircle2 size={20} style={{ color: '#10b981' }} />,           valueColor: '#34d399' },
    ...
    { title: "Approved Days",     value: stats?.totalApprovedDays  || 0, icon: <CalendarDays size={20} style={{ color: '#0293FA' }} />,           valueColor: '#35AAFD' },
  ];
  ...
  return (
    <div className="w-full space-y-8 mb-10">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 text-left">
        <div>
          ...
          <p className="text-xs mt-1" style={{ color: 'var(--text-muted)' }}>
            Manage and track employee leave logs and analytics
          </p>
        </div>
        <div className="z-50">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5">
        {statCards.map((card, index) => (
          <BaseCard key={index} padding="p-5" className="flex flex-col justify-between min-h-[145px] transition-all duration-300 shadow-xl">
            <div className="flex justify-between items-start mb-3">
