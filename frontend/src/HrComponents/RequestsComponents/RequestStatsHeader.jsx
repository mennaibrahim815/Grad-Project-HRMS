...
const RequestStatsHeader = ({ onStatsUpdated, month, year, setMonth, setYear }) => {
  ...
  const statCards = [
    ...
    { title: "Approved Requests", value: stats?.approvedCount || 0, icon: <CheckCircle2 className="text-emerald-500" size={20} />, textColor: "text-emerald-400" },
    { title: "Pending Requests", value: stats?.pendingCount || 0, icon: <Clock className="text-yellow-500" size={20} />, textColor: "text-yellow-400" },
    { title: "Rejected Requests", value: stats?.rejectedCount || 0, icon: <XCircle className="text-red-500" size={20} />, textColor: "text-red-400" }
  ];
    ...
    <div className="w-full space-y-8 mb-10 text-left">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 text-xs mt-1">Review, filter, and respond to general employee requests and operations</p>
        </div>
        <div className="z-50">
        ...
        {statCards.map((card, index) => (
          <BaseCard key={index} padding="p-5" className="flex flex-col justify-between min-h-[145px] hover:border-slate-600/50 transition-all duration-300 shadow-xl text-left">
            <div className="flex justify-between items-start mb-3">
              <span className="text-[10px] font-bold uppercase tracking-[0.1em] text-slate-500">{card.title}</span>
              <div className="p-2 bg-slate-800/40 rounded-xl border border-slate-700/20">{card.icon}</div>
            </div>
