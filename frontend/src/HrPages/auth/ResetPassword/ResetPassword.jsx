          ...
          >
            <div className="w-full max-w-md">
              ...
              <div className="flex items-center gap-2 mb-16">
                ...
                <span style={{ color: 'var(--text-main)' }} className="text-xl font-bold italic">Staf<span className="text-blue-500">fly</span></span>
              </div>

              <Link to="/login" style={{ color: 'var(--text-muted)' }} className="hover:opacity-80 text-sm flex items-center gap-2 mb-6 group transition-all w-fit">
                <i className="fas fa-chevron-left text-xs"></i> Back to login
              </Link>

              <div className="text-center lg:text-left mb-10">
                <h1 style={{ color: 'var(--text-main)' }} className="text-3xl font-bold mb-2 tracking-tight">set a new password</h1>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-2">
                  <label style={{ color: 'var(--text-muted)' }} className="block text-[10px] uppercase font-black tracking-widest ml-1">New Password</label>
                  <div className="relative">
                    <input 
                      ...
                      className={`w-full p-4 rounded-xl border outline-none transition-all ${matchError || error ? 'border-red-500/50' : 'border-transparent focus:border-blue-500'}`}
                    />
                    <button type="button" onClick={() => setShowPass(!showPass)} style={{ color: 'var(--text-muted)' }} className="absolute right-4 top-1/2 -translate-y-1/2 hover:opacity-80">
                ...
                {/* Confirm Password */}
                <div className="space-y-2">
                  <label style={{ color: 'var(--text-muted)' }} className="block text-[10px] uppercase font-black tracking-widest ml-1">Confirm New Password</label>
                  <div className="relative">
                    <input 
                      ...
                      style={{ backgroundColor: 'var(--card-border)', color: 'var(--text-main)' }}
                      className={`w-full p-4 rounded-xl border outline-none transition-all ${matchError ? 'border-red-500/50 focus:border-red-500' : 'border-transparent focus:border-blue-500'}`}
                    />
                    <button type="button" onClick={() => setShowConfirmPass(!showConfirmPass)} style={{ color: 'var(--text-muted)' }} className="absolute right-4 top-1/2 -translate-y-1/2 hover:opacity-80">
          ...
          <AnimatePresence>
            {success && (
              <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
                ...
                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-main)' }} className="relative border p-10 rounded-[2.5rem] shadow-2xl max-w-sm w-full text-center">
                  ...
                  <h3 style={{ color: 'var(--text-main)' }} className="text-2xl font-black mb-3">Successful</h3>
                  <p style={{ color: 'var(--text-muted)' }} className="text-sm mb-8 leading-relaxed font-medium">Your password has been changed. You can now use it to login.</p>
                  <button 
