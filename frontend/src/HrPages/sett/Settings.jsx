...
const Settings = () => {
  ...
  useEffect(() => {
    dispatch(fetchSettings()); // أول ما يعمل ريفريش هيروح يجيب الداتا الجديدة من الداتا بيز
  }, [dispatch]);
  ...
  return (
    ...
    <div className="max-w-[1450px] mx-auto p-4 md:p-8 text-white h-[calc(100vh)] flex flex-col overflow-hidden">
      ...
      <div className="flex flex-col min-[900px]:flex-row flex-1 gap-6 md:gap-8 overflow-hidden">
            ...
            >
              <div
                className={`w-10 h-10 md:w-11 md:h-11 rounded-xl md:rounded-2xl shrink-0 flex items-center justify-center text-base md:text-lg ${
                  activeTab === item.id
                    ? "bg-[#1e293b] text-blue-400 shadow-[0_0_15px_rgba(59,130,246,0.2)]"
                    : "bg-[#142129] text-gray-500"
                }`}
              >
              ...
              <div className="min-w-0">
                <h4
                  className={`text-xs md:text-sm font-bold truncate ${activeTab === item.id ? "text-white" : "text-gray-400"}`}
                >
                  {item.title}
                </h4>
                <p className="text-[9px] md:text-[10px] text-gray-600 font-medium truncate hidden md:block">
                  {item.desc}
                </p>
              </div>
            </button>
        ...
        <main className="flex-1 bg-[#142129] h-[100%] rounded-3xl md:rounded-[2.5rem] border border-gray-800/40 shadow-2xl overflow-hidden flex flex-col relative">
