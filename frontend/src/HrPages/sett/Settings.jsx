...
// استيراد الأكشنز لجلب الداتا الحقيقية
import { fetchMyHRProfile } from '../../store/HrSlices/navbar/hrProfileSlice';
// افترضت إنك عملتي سلايس لإعدادات الشركة
// import { fetchCompanySettings } from '../../store/HrSlices/settings/settingsSlice';
...
const Settings = () => {
  ...
  return (
    // h-[calc(100vh-100px)] بتضمن إن الصفحة متعديش طول الشاشة وتفعل السكرول الداخلي
    <div className="max-w-[1450px] mx-auto p-4 md:p-8 text-white h-[calc(100vh-40px)] flex flex-col overflow-hidden">
      ...
      <div className="flex flex-1 gap-8 overflow-hidden">
        {/* سـايد بار الإعدادات - ثابت (No Scroll) */}
        <aside className="hidden lg:flex flex-col w-[380px] space-y-3 shrink-0">
          {menuItems.map((item) => (
            <button
              key={item.id}
              onClick={() => setActiveTab(item.id)}
        ...
        {/* محتوى التاب - متحرك (Scrollable) */}
        <main className="flex-1 bg-[#142129] rounded-[2.5rem] border border-gray-800/40 shadow-2xl overflow-hidden flex flex-col relative">
              ...
              >
                ...
                {activeTab === 'access' && (
                   <div className="py-20 text-center opacity-30 italic">
                      ...
                      <p>Access Control & Integration settings coming soon...</p>
                   </div>
                )}
              </motion.div>
            </AnimatePresence>
