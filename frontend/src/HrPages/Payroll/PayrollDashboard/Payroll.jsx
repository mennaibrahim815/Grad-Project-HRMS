...
export default function Payroll() {
  ...
  const { analytics, loading,  selectedMonth} = useSelector((state) => state.payroll);

  useEffect(() => {
    const [year, month] = selectedMonth.split("-");
    dispatch(fetchPayrollSummary({ month: parseInt(month), year: parseInt(year) }));
  }, [dispatch, selectedMonth]);

  const summaryCards = analytics?.data?.summaryCards;
  const payrollRaw= analytics?.data?.paymentStatusChart;
  const payrollChartData = [
  { name: "Paid", value: payrollRaw?.paidCount || 0, color: "#10b981", label: "Paid" },
  { name: "Pending", value: payrollRaw?.pendingCount || 0, color: "#3b82f6", label: "Pending" },
  { name: "Draft", value: payrollRaw?.draftCount || 0, color: "#4b5563", label: "Draft" },
];
