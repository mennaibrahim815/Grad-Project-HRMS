...
export default function Project() {
  ...
  // الفلترة بناءً على حقل status المباشر من الـ root
  const distributeProjects = (projectsList) => {
    const newColumns = {
      "On-going": projectsList.filter(p => p.status && p.status.toLowerCase() === "on-going"),
      Pending: projectsList.filter(p => p.status && p.status.toLowerCase() === "pending"),
      Completed: projectsList.filter(p => p.status && p.status.toLowerCase() === "completed"),
    };
    setColumns(newColumns);
  };
            ...
            {Object.keys(columns).map((col) => (
              <Column
                ...
                tasks={columns[col].map(p => ({
                  ...
                  // تـمّ تمرير مصفوفة الـ subTasks هنا لتصل لكارت العرض بنجاح
                  subTasks: p.subTasks || [], 
